...
class PurkinjeCell(bp.dyn.NeuDyn):
    def __init__(self, size, **kwargs):
        ...
        self.cspk = bm.Variable(bm.zeros(self.num, dtype=bool)) # indices of PCs receiving IO spike
        self.rho =bm.Variable(bm.zeros(self.num)) # instanteneous firing rates in 200ms window
        self.tau_rate = kwargs["tau_rate"]
    ...
    def update(self):
        ...
        self.t_last_spike.value = bm.where(spike, t, self.t_last_spike) # If spike update time.

        # Instantaneous firing rate in kHz
        self.rho.value = self.rho.value * bm.exp((-1.0*dt) /self.tau_rate) + (spike / dt) * (1.0 - bm.exp((-1.0*dt) / self.tau_rate))
